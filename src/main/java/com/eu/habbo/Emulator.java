...
public final class Emulator {
    ...
    public static final String version = "Arcturus Morningstar Reworked" + " " + MAJOR + "." + MINOR + "." + BUILD + " " + PREVIEW;
        ...
        while (m.find()) {
            try {
                ...
                if (map.containsKey(what)) {
                    ...
                    LOGGER.warn("Unknown time unit: {}", what);
                }
            }
            catch (NumberFormatException e) {
                LOGGER.warn("Invalid time format in timeString: {}", m.group(2), e);
            }
            catch (Exception e) {
                LOGGER.error("Unexpected error processing time string", e);
            }
        }
        ...
        while (m.find()) {
            try {
                ...
                if (map.containsKey(what)) {
                    ...
                    LOGGER.warn("Unknown calendar field: {}", what);
                }
            }
            catch (NumberFormatException e) {
                LOGGER.warn("Invalid number format in date modifier: {}", m.group(2), e);
            }
            catch (Exception e) {
                LOGGER.error("Unexpected error modifying date", e);
            }
        }
