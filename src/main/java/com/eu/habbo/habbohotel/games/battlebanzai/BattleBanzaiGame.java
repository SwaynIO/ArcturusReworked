...
public class BattleBanzaiGame extends Game {
    ...
    private synchronized void resetMap() {
        ...
        // Optimized: Use specialized collections and batch state updates
        Collection<HabboItem> floorItems = this.room.getFloorItems();
        List<HabboItem> tilesToUpdate = new ArrayList<>(floorItems.size() / 4); // Estimated size
        List<HabboItem> scoreboardsToUpdate = new ArrayList<>();
        
        for (HabboItem item : floorItems) {
            if (item instanceof InteractionBattleBanzaiTile) {
                item.setExtradata("1");
                tilesToUpdate.add(item);
                this.tileCount++;
                this.gameTiles.put(item.getId(), item);
            } else if (item instanceof InteractionBattleBanzaiScoreboard) {
                item.setExtradata("0");
                scoreboardsToUpdate.add(item);
            }
        }
        
        // Batch update item states to reduce room update overhead
        for (HabboItem item : tilesToUpdate) {
            this.room.updateItemState(item);
        }
        for (HabboItem item : scoreboardsToUpdate) {
            this.room.updateItemState(item);
        }
    }
