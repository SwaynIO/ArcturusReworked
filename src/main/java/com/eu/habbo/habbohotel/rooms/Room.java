...
public class Room implements Comparable<Room>, ISerialize, Runnable {
    ...
    // Optimized initial capacity based on typical room usage (16 users average)
    private final ConcurrentHashMap<Integer, Habbo> currentHabbos = new ConcurrentHashMap<>(16, 0.75f, 2);
    ...
    private synchronized void loadItems(Connection connection) {
        ...
        // Optimized: Load items first, then wired data separately for better performance
        try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM items WHERE room_id = ? ORDER BY id")) {
            statement.setInt(1, this.id);
            ...
            LOGGER.error("Caught SQL exception during item loading", e);
        }
        
        // Load wired data in separate optimized query
        try (PreparedStatement statement = connection.prepareStatement("SELECT id, wired_data FROM items WHERE room_id = ? AND wired_data IS NOT NULL AND wired_data != ''")) {
            statement.setInt(1, this.id);
            try (ResultSet set = statement.executeQuery()) {
                while (set.next()) {
                    try {
                        ...
                        LOGGER.error("Failed to load wired data for item {}", set.getInt("id"), e);
                    }
                }
            }
        } catch (SQLException e) {
            LOGGER.error("Caught SQL exception during wired data loading", e);
        }
    ...
    private synchronized void loadBots(Connection connection) {
        ...
        // Optimized: Use index on room_id and limit result set processing
        try (PreparedStatement statement = connection.prepareStatement("SELECT users.username AS owner_name, bots.* FROM bots INNER JOIN users ON bots.user_id = users.id WHERE bots.room_id = ? ORDER BY bots.id")) {
            statement.setInt(1, this.id);
            try (ResultSet set = statement.executeQuery()) {
                // Pre-calculate array bounds for safety
                int maxDanceType = DanceType.values().length - 1;
                int maxRotation = RoomUserRotation.values().length - 1;
                ...
                while (set.next()) {
                    ...
                    if (b != null) {
                        ...
                        // Optimized tile lookup with bounds checking
                        RoomTile botTile = this.layout.getTile((short) set.getInt("x"), (short) set.getInt("y"));
                        if (botTile == null) {
                            botTile = this.getLayout().getDoorTile();
                            b.getRoomUnit().setLocation(botTile);
                            ...
                            int rotValue = set.getInt("rot");
                            b.getRoomUnit().setRotation(rotValue >= 0 && rotValue <= maxRotation ? 
                                RoomUserRotation.values()[rotValue] : RoomUserRotation.NORTH);
                        }
                        
                        b.getRoomUnit().setRoomUnitType(RoomUnitType.BOT);
                        
                        // Safe array access for dance type
                        int danceValue = set.getInt("dance");
                        b.getRoomUnit().setDanceType(danceValue >= 0 && danceValue <= maxDanceType ? 
                            DanceType.values()[danceValue] : DanceType.NONE);
            ...
            LOGGER.error("Caught SQL exception during bot loading", e);
        }
    }
    ...
    // Cache the values collection to avoid creating new collection on every call
    private volatile Collection<Habbo> habbosCache;
    private volatile long habbosCacheTime;
    
    public Collection<Habbo> getHabbos() {
        // Simple cache with 100ms TTL to balance performance vs consistency
        long currentTime = System.currentTimeMillis();
        if (habbosCache == null || currentTime - habbosCacheTime > 100) {
            habbosCache = this.currentHabbos.values();
            habbosCacheTime = currentTime;
        }
        return habbosCache;
    }
    
    // Invalidate cache when users change
    private void invalidateHabbosCache() {
        habbosCache = null;
    }
    ...
    public void addHabbo(Habbo habbo) {
        synchronized (this.roomUnitLock) {
            ...
            this.unitCounter++;
            this.updateDatabaseUserCount();
            this.invalidateHabbosCache(); // Invalidate cache when user added
        }
    }
    ...
    public void removeHabbo(Habbo habbo, boolean sendRemovePacket) {
        ...
        synchronized (this.roomUnitLock) {
            ...
            this.invalidateHabbosCache(); // Invalidate cache when user removed
        }
