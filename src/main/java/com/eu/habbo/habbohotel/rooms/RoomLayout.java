...
public class RoomLayout {
    ...
    /// Further optimized by SailorEudes for Arcturus Morningstar Reworked - Performance Enhanced
    public final Deque<RoomTile> findPath(RoomTile oldTile, RoomTile newTile, RoomTile goalLocation, RoomUnit roomUnit, boolean isWalktroughRetry) {
        ...
        // Optimized: Use PriorityQueue for openList (O(log n) vs O(n) for LinkedList)
        // Use HashSet for closedList (O(1) lookup vs O(n) for LinkedList.contains())
        PriorityQueue<RoomTile> openList = new PriorityQueue<>(Comparator.comparingDouble(RoomTile::getfCosts));
        Set<RoomTile> closedSet = new HashSet<>(64); // Pre-sized for typical room navigation
        
        RoomTile startTile = oldTile.copy();
        ...
        // Cache for faster timeout checking (avoid System.currentTimeMillis() spam)
        int maxExecutionTime = Emulator.getConfig().getInt("pathfinder.execution_time.milli", 25);
        boolean timeoutEnabled = Emulator.getConfig().getBoolean("pathfinder.max_execution_time.enabled", false);
        int iterations = 0;
        
        while (!openList.isEmpty()) {
            // Check timeout less frequently for better performance (every 10 iterations)
            if (timeoutEnabled && (++iterations % 10 == 0) && 
                System.currentTimeMillis() - startMillis > maxExecutionTime) {
                return null;
            }

            // Optimized: PriorityQueue automatically gives us lowest F cost
            RoomTile current = openList.poll();
            if (current.x == newTile.x && current.y == newTile.y) {
                return this.calcPath(startTile, current);
            }
            ...
            for (RoomTile currentAdj : adjacentNodes) {
                // Optimized: O(1) lookup with HashSet instead of O(n) with LinkedList
                if (closedSet.contains(currentAdj)) continue;
