...
public class RoomChatMessage implements Runnable, ISerialize, DatabaseLoggable {
    ...
    public RoomChatMessage(MessageHandler message) {
        ...
        // Optimized: Only check banned bubbles if there are any and user doesn't have permission
        if (RoomChatMessage.BANNED_BUBBLES.length > 0 && !message.client.getHabbo().hasPermission(Permission.ACC_ANYCHATCOLOR)) {
            int bubbleType = this.bubble.getType();
            for (Integer bannedBubble : RoomChatMessage.BANNED_BUBBLES) {
                if (bannedBubble == bubbleType) {
                    this.bubble = RoomChatMessageBubbles.NORMAL;
                    break;
                }
            }
    ...
    public void filter() {
        // Optimized: Early exit and cached permission checks
        boolean hasClubActive = this.habbo.getHabboStats().hasActiveClub();
        boolean hasFilterPermission = this.habbo.hasPermission(Permission.ACC_CHAT_NO_FILTER);
        
        // Remove chat colors for non-club users with optimized string operations
        if (!hasClubActive && !chatColors.isEmpty()) {
            String originalMessage = this.message;
            for (String chatColor : chatColors) {
                if (originalMessage.contains(chatColor)) {
                    this.message = this.message.replace(chatColor, "");
                }
            }
        }

        // Optimized: Cache config lookups and use early returns
        boolean wordFilterEnabled = Emulator.getConfig().getBoolean("hotel.wordfilter.enabled");
        boolean roomFilterEnabled = Emulator.getConfig().getBoolean("hotel.wordfilter.rooms");
        
        if (wordFilterEnabled && roomFilterEnabled && !hasFilterPermission) {
            var wordFilter = Emulator.getGameEnvironment().getWordFilter();
            
            if (wordFilter.autoReportCheck(this)) {
            ...
            if (!wordFilter.hideMessageCheck(this.message)) {
                wordFilter.filter(this, this.habbo);
                return;
            }
