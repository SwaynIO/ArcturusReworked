...
public class RoomManager {
    ...
    private final List<Class<? extends Game>> gameTypes;

    public RoomManager() {
        long millis = System.currentTimeMillis();
        this.roomCategories = new ConcurrentHashMap<>();
        this.mapNames = Collections.synchronizedList(new ArrayList<>());
        this.activeRooms = new ConcurrentHashMap<>();
        this.loadRoomCategories();
        this.loadRoomModels();

        this.gameTypes = Collections.synchronizedList(new ArrayList<>());
    ...
    public THashMap<Integer, List<Room>> findRooms(NavigatorFilterField filterField, String value, int category, boolean showInvisible) {
        ...
        StringBuilder queryBuilder = new StringBuilder(filterField.databaseQuery);
        queryBuilder.append(" AND rooms.state NOT LIKE ?");
        ...
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection(); 
            ...
            statement.setString(2, showInvisible ? "" : "invisible");
            
            int currentParam = 3;
            if (category >= 0) {
                statement.setInt(currentParam++, category);
            }
            
            int offset = page * NavigatorManager.MAXIMUM_RESULTS_PER_PAGE;
            statement.setInt(currentParam++, offset);
            statement.setInt(currentParam, NavigatorManager.MAXIMUM_RESULTS_PER_PAGE);
