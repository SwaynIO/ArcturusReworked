...
public class HabboManager {
    ...
    // Cache for offline user info - 10 minute TTL to improve performance
    private static final CacheManager<Integer, HabboInfo> OFFLINE_USER_CACHE = 
        new CacheManager<>(10 * 60 * 1000L); // 10 minutes
    
    // Cache for offline user info by username - 10 minute TTL
    private static final CacheManager<String, HabboInfo> OFFLINE_USER_USERNAME_CACHE = 
        new CacheManager<>(10 * 60 * 1000L); // 10 minutes
    ...
    public static HabboInfo getOfflineHabboInfo(int id) {
        // Use cache to avoid repeated database queries for same user
        return OFFLINE_USER_CACHE.get(id, () -> {
            HabboInfo info = null;
    ...
    public static HabboInfo getOfflineHabboInfo(String username) {
        ...
        // Use cache to avoid repeated database queries for same username
        return OFFLINE_USER_USERNAME_CACHE.get(username.toLowerCase(), () -> {
            HabboInfo info = null;
            try (Connection connection = Emulator.getDatabase().getDataSource().getConnection(); 
                ...
                try (ResultSet set = statement.executeQuery()) {
                    if (set.next()) {
                        ...
                        if (info != null) {
                            OFFLINE_USER_CACHE.put(info.getId(), info);
                        }
                    }
                }
            } catch (SQLException e) {
