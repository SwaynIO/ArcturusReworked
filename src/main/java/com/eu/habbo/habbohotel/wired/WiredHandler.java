...
public class WiredHandler {
    ...
    public static boolean handle(WiredTriggerType triggerType, RoomUnit roomUnit, Room room, Object[] stuff) {
        ...
        // Pre-sized collections for better performance (typical wired setups)
        THashSet<InteractionWiredEffect> effectsToExecute = new THashSet<>(16);

        List<RoomTile> triggeredTiles = new ArrayList<>(8);
        for (InteractionWiredTrigger trigger : triggers) {
            ...
            // Pre-sized collection for effects
            THashSet<InteractionWiredEffect> tEffectsToExecute = new THashSet<>(8);
