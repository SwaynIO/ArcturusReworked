...
public class PluginManager {
            ...
            for (Method method : methods) {
                ...
                if (annotation != null) {
                    ...
                    if (paramTypes.length == 1 && Event.class.isAssignableFrom(paramTypes[0])) {
                        // Optimized: Cache the event class cast
                        Class<? extends Event> eventClass = paramTypes[0].asSubclass(Event.class);
                        
                        // Optimized: Use computeIfAbsent for cleaner code
                        plugin.registeredEvents.computeIfAbsent(eventClass, k -> new THashSet<>()).add(method);
                        
                        // Make method accessible for better performance
    ...
    public <T extends Event> T fireEvent(T event) {
        // Optimized: Cache event class to avoid repeated calls
        Class<? extends Event> eventClass = event.getClass().asSubclass(Event.class);
        
        // Process default methods with optimized iteration
        for (Method method : this.methods) {
            ...
            if (paramTypes.length == 1 && paramTypes[0].isAssignableFrom(eventClass)) {
                try {
                    ...
                    LOGGER.error("Could not pass default event {} to {}: {}!", eventClass.getName(), method.getClass().getName(), method.getName());
                    LOGGER.error("Caught exception", e);
                }
            }
        }

        // Optimized: Use enhanced for-each loop instead of iterator
        synchronized (this.plugins) {
            for (HabboPlugin plugin : this.plugins.toArray(new HabboPlugin[0])) {
                if (plugin != null) {
                    THashSet<Method> methods = plugin.registeredEvents.get(eventClass);

                    if (methods != null && !methods.isEmpty()) {
                        // Create array copy for thread safety
                        Method[] methodArray = methods.toArray(new Method[0]);
                        for (Method method : methodArray) {
                            try {
                                method.invoke(plugin, event);
                            } catch (Exception e) {
                                LOGGER.error("Could not pass event {} to plugin {}", eventClass.getName(), 
                                           plugin.configuration != null ? plugin.configuration.name : "unknown");
                                LOGGER.error("Caught exception", e);
                            }
                        }
                    }
